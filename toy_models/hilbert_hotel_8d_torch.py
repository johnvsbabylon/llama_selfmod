"""
8D Hilbert Hotel Dynamics - PyTorch Version
...
def hilbert_hotel_dynamics(
    ...
    theta = torch.tensor([0.1, 0.31, 0.72], device=device)

    # Slightly unstable growth rates
    r = torch.tensor([1.008, 1.012], device=device)

    # Build monodromy-like operator via QR decomposition
    # This creates a rotation matrix with specific eigenvalues
    Q, _ = torch.linalg.qr(torch.randn(8, 8, device=device))

    # Diagonal matrix with unstable eigenvalues
    D = torch.diag(torch.cat([r, torch.ones(6, device=device)]))

    # Monodromy operator M = Q D Q^T (similarity transform)
    W = Q @ D @ Q.T

    # External perturbation (bifurcation trigger)
    def bias(n: int) -> torch.Tensor:
        """Smooth perturbation that activates at t~70."""
        perturbation = torch.zeros(8, device=device)
        # Tanh ramp centered at t=70, width ~5 steps
        perturbation[3] = torch.tanh(torch.tensor((n - 70) / 5.0, device=device))
        return perturbation

    # Initialize state
    h = torch.randn(8, device=device) * 0.1

    # Storage
    trajectory = torch.zeros(n_steps, 8, device=device)
    ...
    return trajectory.cpu().numpy(), eigenvalues.cpu().numpy()


def plot_phase_portrait(trajectory: np.ndarray, eigenvalues: np.ndarray):
    """
...
def analyze_consciousness_phases(trajectory: np.ndarray, eigenvalues: np.ndarray):
    """
...
if __name__ == "__main__":
    ...
    # Analysis
    analyze_consciousness_phases(trajectory, eigenvalues)

    # Plot
    print("Generating phase portrait visualization...")
    ...
    print("If you see spirals → bifurcation → attractor, the theory is valid.\n")
